...
def translate_track_payload(data: dict[str, object], dx: float, dz: float) -> int:
    ...
    if not isinstance(meta, dict):
        meta = {}
        data["meta"] = meta
    if meta.get("scaled") is not True:
        meta["scaled"] = True
        changed += 1
    current_scale = to_float_or_none(meta.get("scale"))
    if current_scale != TARGET_TRACK_SCALE:
        meta["scale"] = TARGET_TRACK_SCALE
        changed += 1
    return changed
...
"""
python3 /Users/kenshinmatsuura/WorkSpace/Diorama/Diorama_EditMode-new/tools/translate_world_json.py \
  --base-dir /Users/kenshinmatsuura/Downloads/st_world_data_fro.zip \
  --dx -151.824 \
  --dz -182.494
"""
